...
from ..utils.fourier_process import Unify2fourier1D, sample_contour_theta_rmax, Polar2Cartesian
...
class ContourPointDataset(data.Dataset):
    ...
    def _obtain_ms_regression(self, down_ratio, img, img_id, c, s, height, width, rot_ins, fliped_flag, inp, org_w,org_h):
        ...
        contourpoint = np.zeros((self.max_objs, self.cfg.train.ns), dtype=np.float32)  # 26 = (2 * fd + 1) *2
        ...
        for k in range(num_objs):
            ...
            if 0 <= center[0] <= out_w - 1 and 0 <= center[1] <= out_h - 1 and wh[0] > 1 and wh[1] > 1 and len(theta_d) >= self.cfg.train.ns:
                ...
                #fourier_signture, r_max, _ = Unify2fourier1D(theta_d, fd=self.cfg.train.fd)

                contourpoint[k] = theta_d[:, -1]
...
#                from fourier_process import fourier2poly_rmax, Polar2Cartesian
                ...
                # #
                # res_poly = fourier2poly_rmax(c, recon_points=self.cfg.train.ns).reshape(-1)
                # #
                # before_points = res_poly * r_max / 10
                #
                # after_points = Polar2Cartesian((center[0], center[1]), theta_d[:, -1], self.cfg.train.ns)
                # #
                # from tool import plot_points
