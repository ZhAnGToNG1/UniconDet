...
from ..utils.fourier_process import Unify2fourier1D_wo_norm, sample_contour_theta_rmax
...
class UnifyDataset_1DRegion_wonorm(data.Dataset):
    ...
    def _obtain_ms_regression(self, down_ratio, img, img_id, c, s, height, width, rot_ins, fliped_flag, inp, org_w,org_h):
        ...
        for k in range(num_objs):
            ...
            #---------------
            if 0 <= center[0] <= out_w - 1 and 0 <= center[1] <= out_h - 1 and wh[0] > 1 and wh[1] > 1:
                ...
                fourier_signture, _ = Unify2fourier1D_wo_norm(theta_d, fd=self.cfg.train.fd)
