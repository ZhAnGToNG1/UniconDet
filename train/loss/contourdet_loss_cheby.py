from .losses import FocalLoss_hm, SmoothL1Loss, FourierLoss_rmax, Polarmask_loss,FourierLoss_rmax_weight,ChebyLoss_rmax
import torch
import torch.nn as nn
from utils.tool import _sigmoid

class ContourDet_loss_cheby(nn.Module):
    def __init__(self, cfg):
        super(ContourDet_loss_cheby,self).__init__()
        self.hm_func = FocalLoss_hm()
        self.reg_func = SmoothL1Loss()
        self.rmax_func = SmoothL1Loss()
        self.cheby_func = ChebyLoss_rmax(cfg)
        self.cfg = cfg
    ...
    def forward(self, outputs, batches):
        ...
        for i in range(len(outputs)):
            ...
            fourier_loss += self.cheby_func(output['fourier'],batch['reg_mask'],batch['ind'], batch['fourier'])
