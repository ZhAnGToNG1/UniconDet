...
class eval_isaid(object):
    def __init__(self, cfg, dataloder):
        self.cfg = cfg
        self.save_path = cfg.val.inter_save_path
        self.dataloader = dataloder
        self.evalator = Evaluator(cfg, self.cfg.val.inter_save_path, self.cfg.data.gt_path)
    ...
    def test_in_training(self, model):
        ...
        for batch in tqdm(self.dataloader):
            inp = batch[0]['input'].cuda()
            with torch.no_grad():
                ret = validator.run(inp, model)
            ...
            self.evalator.evaluate(output, batch)
        return self.evalator.summarize()
...
class Evaluator:
    ...
    def evaluate(self, output, batch):
        ...
        img_id = int(batch[0]['meta']['img_id'])
        center = batch[0]['meta']['center'].detach().cpu().numpy()
        scale = batch[0]['meta']['scale'].detach().cpu().numpy()

        h, w = batch[0]['input'].size(2), batch[0]['input'].size(3)
    ...
    def summarize(self):
        ...
        coco_eval.evaluate()
        coco_eval.accumulate()
        stats, print_coco = coco_eval.summarize()
        class_index = []
